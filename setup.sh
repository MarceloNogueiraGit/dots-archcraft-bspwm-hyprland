...
for PKG1 in alacritty-git alacritty-themes zsh brave-bin neofetch neovim-nightly python-pynvim wl-clipboard xclip xsel swaybg swaylock-effects wofi wlogout mako grim slurp wl-clipboard polkit-kde-agent nwg-look-bin swww mousepad micro pavucontrol playerctl; do
...
# check for existing config folders and backup 
    for DIR in btop cava hypr alacritty nvim foot mako swaylock waybar wlogout wofi 
    do 
        DIRPATH=~/.config/$DIR
        if [ -d "$DIRPATH" ]; then 
